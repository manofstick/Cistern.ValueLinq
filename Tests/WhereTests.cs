...
namespace Cistern.ValueLinq.Tests
{
    public class WhereTests : EnumerableTests
    {
        ...
        [Fact(Skip="CISTERN.VALUELINQ: Hmmm. I *could* do this with a temp value stored in the Where node, but isn't this just undefined behaviour?")]
        ...
        {
            int[] source = new[] { 1, 2, 3, 4, 5 };
            IEnumerator<int> enumerator = ((IEnumerable<int>)source.Where(value => true)).GetEnumerator();
        ...
        [Fact(Skip="CISTER.VALUELINQ: Not valid")]
...
#if CISTERN_VALUELINQ_NOT_VALID
        [Fact]
        ...
        {
            foreach (var equivalent in new IEnumerable<int>[] { source.Where(s => true), source.Where(s => true).Select(s => s) })
            {
